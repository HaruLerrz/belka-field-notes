# Resume Lite

本頁整理學經歷、工作範圍、使用工具與資格摘要，作為 Belka Field Notes 的簡歷入口。

## Profile

我是 Jack／HaruLerrz，具影音新聞、攝影、內容製作、社群經營與公共議題整理經驗。

過去工作內容包含新聞現場拍攝、素材整理、影片剪輯、口白腳本、YouTube 上架與內容包裝；也長期參與粉專經營、社群貼文、迷因內容、公共議題長文與現場短講。

近年另將 AI 工具導入內容流程與個人實作，使用 Whisper、Ultimate Vocal Remover、Stable Diffusion、Modly、GPT-SoVITS 與大型語言模型處理轉錄、音源分離、資料整理、腳本輔助、配音測試、3D 模型與材質測試、prompt workflow，以及小型工具與互動敘事系統的原型開發。

## Work Experience

### 壹傳媒媒體控股股份有限公司（原毅傳媒控股股份有限公司）

**攝影記者｜2022-09 ～ 2025-11**

* 參與即時新聞、專題、生活與消費類影音內容製作。
* 負責新聞現場拍攝、素材整理、影片製作與內容上架支援。
* 2023 年 2 月至 2025 年 11 月期間，主導短影音新聞 VTuber「壹傳子」企劃與製作，將 VTuber 技術導入即時V新聞內容。
* 角色立繪以 Stable Diffusion 生成，並使用 Live2D Cubism 完成角色建模、透過 VTube Studio 進行動作捕捉，再結合新聞素材、文字、影音剪輯與 AI 播報語音完成短篇新聞影片。
* 依 Shorts／短影音約一分鐘限制，將新聞資料壓縮成可完成口播的篇幅，保留必要數字、地名、人名與背景資訊。
* 製作 YouTube 長片、汽車發表、旅遊、美食與醫療人物內容。
* 依題材需求整理資料、撰寫口白、規劃畫面並處理影片剪輯。
* 熟悉新聞時效、標題包裝、YouTube 描述與平台呈現需求。
### 社群與粉專經營

* 長期經營與協助多個 Facebook 粉絲專頁，內容涵蓋次文化、迷因、音樂、校園與公共議題。
* 曾參與最高約 12 萬讚粉專，以及 6 萬讚、1 萬讚級粉專的內容與社群工作。
* 具貼文規劃、內容切角、互動觀察、受眾反應與觸及／轉換分析經驗。
* 曾整理粉專成長、迷因自然擴散與高觸及低轉換等案例。

### 公共議題整理與表達

* 長期於 Threads 與 Facebook 撰寫公共議題整理文與長文。
* 主題包含數發部、網路安全憑證、台灣史資料、國會擴權、憲政程序、司法政治事件與矯正制度。
* 能將法政、技術與制度議題整理為時間線、問答式說明、懶人包或口語稿。
* 青鳥行動期間曾進行現場短講，具公共議題口語化與群眾表達經驗。

## Education

### 國立陽明交通大學

**傳播研究所碩士班｜碩士｜2021-10**

正式學位為傳播研究所碩士；入學時以傳播研究所乙組名義招生，實際於傳播與科技學系就讀。該組為現今傳播與科技學系碩士班的前身，後續停止以傳播所乙組名義代招，改由傳播與科技學系獨立招生。

研究方向包含：

* 社群關係
* 次文化迷群
* 社群認同
* 傳播與社群分析

### 國立政治大學

**哲學系｜學士｜2013-06**  
**輔系：日本語文學系**

## Certifications

* 日本語能力試驗 JLPT N1／N2 合格
* Master of Communication Professional（MCP）專業認證｜International Organization of Professional Cultivation and Accreditation（IOPCA）

## Areas of Work

### Media / Content

* 新聞攝影與現場拍攝
* 影片剪輯與素材整理
* 口白腳本與內容包裝
* YouTube 上架、標題與描述整理
* 長片、短影音與新聞流程支援
* VTuber 新聞角色製作與短影音採編流程

### Social / Public Issues

* 粉專經營與社群貼文
* 次文化與迷因內容
* 受眾互動與觸及分析
* 公共議題與法政資料整理
* 現場短講與口語轉譯

### AI-assisted Workflow

* Whisper 轉錄與內容整理
* Ultimate Vocal Remover 音源分離、語音抽取與高噪音現場素材清理
* Stable Diffusion 圖像生成
* Modly 3D 模型生成、Hunyuan 材質管線啟用與原生擴充元件編譯
* GPT-SoVITS 語音與配音測試
* prompt workflow 與模組化指令
* AI 輔助程式撰寫、除錯與文件整理
* Node.js、PowerShell、Python 與瀏覽器 console 小型工具
* 互動敘事系統與狀態管理原型

## Tools

### Video / Audio

#### Video Editing / Stabilization / Subtitling

* MAGIX VEGAS Pro
* proDAD Mercalli
* Subtitle Edit

#### Audio / Speech

* Audacity
* Whisper
* Ultimate Vocal Remover
* GPT-SoVITS

#### Streaming / Virtual Production

* Streamlabs Desktop
* VTube Studio

### Graphics / Layout

#### Image Editing / Photography

* Affinity Photo 2
* Luminar Neo

#### Illustration / Design / Publishing

* Clip Studio Paint
* Affinity Designer 2
* Affinity Publisher 2

#### Character Rigging / Generative Media / 3D

* Live2D Cubism
* Stable Diffusion
* Modly

### Scripting / Web Prototyping

* HTML / CSS
* JavaScript / Node.js
* Python
* PowerShell scripting
* Windows CMD / batch scripting

## Languages

* 繁體中文：母語
* 日文：JLPT N1／N2 合格，可閱讀日文新聞、社群內容與資料
* 英文：可閱讀一般技術文件與操作說明

## Contact

**Email:** [jacksunexe@gmail.com](mailto:jacksunexe@gmail.com)  
**完整影音作品集：** [Jack Sun's Works｜Linktree](https://linktr.ee/jacksunexe)

## Navigation

* [查看 Selected Works](works.md)
* [返回根 README](../README.md)
