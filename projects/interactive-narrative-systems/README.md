...
本頁整理兩個 AI 互動敘事／文字遊戲專案，重點放在共通系統架構、個別設計、狀態管理、prompt packaging、API 串接、UI 迭代與使用者回饋。
...
2025 年 5 月，我曾向 AI 平台服務 詢問 API 是否能提供 stateless request、記憶重置或個別 session 隔離。當時客服回覆尚未支援此類功能。

近期模型平台增加歷史訊息數量限制與跨對話 memory 開關後，我也立即將這些設定加入目前版本，進一步降低平台端既有上下文干擾本地 `lastStatus` 的可能性。
...
| API 平台端仍可能帶入既有對話脈絡 | API／context control | 在 AI 平台服務 提供相關功能後，加入 history limit 與跨對話 memory 關閉設定 |
...
Node.js, Express, prompt packaging, AI-assisted prototyping, issue tracing, source verification, public request workflow, interactive narrative, text-based game, status persistence, session management, output parsing, fallback extraction, request-scoped prompt, UI iteration, web prototype, hosting maintenance
