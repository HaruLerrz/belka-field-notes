# Case Notes

這裡依問題、工作流程與實際結果，整理影音製作、社群經營、公共議題、網站建置、開源貢獻與數位工具原型等案例。

## Case Index

### [Media Workflow](media-workflow.md)

整理壹傳子／即時V新聞、長片、靜態新車發表、轉錄、音源分離與影音上架流程。

### [Public Issue Writing](public-issue-writing.md)

整理公共議題、法政資料、制度爭議與社群長文的資料查核及轉譯方式。

### [Social Media Cases](social-media-cases.md)

整理粉專經營、迷因擴散、觸及與轉換，以及公共知識內容推廣案例。

### [Digital Workflow Prototyping](digital-workflow-prototyping.md)

整理小型工具、AI 輔助原型、實機測試、錯誤修正與文件化流程。

### [Legacy Notepad Upstream Contributions](legacy-notepad-upstream-pr.md)

整理從 Windows Notepad 使用痛點出發，進入 C++ / Win32 開源 codebase，完成 zh-TW、設定保存、RichEdit 搜尋／貼上／取代與鍵盤操作修正，並拆成多個 upstream PR 的實測流程。

### [Microsoft Word 1.1a x64 Compatibility Experiments](word-1-1-x64-compatibility.md)

整理既有 Word 1.1a native x64 port fork 的 build、legacy C / Win32 相容性、字型 rendering／Unicode 排查，以及以 branch／worktree 隔離修正、build 與實機驗證的過程。

### [Belka 3D Showcase](belka-3d-showcase.md)

整理 3D 展示站、Windows 管理工具、部署流程、載入最佳化、跨平台 ZIP 排查與開源準備。

### [Enabling Hunyuan Texture Generation in Modly](enabling-hunyuan-texture-generation-in-modly.md)

整理 Hunyuan3D Mini 介面未提供的 Texture 流程、相依套件、Windows 原生元件編譯、Diffusers 相容性、8GB VRAM offload、runtime 匯入路徑驗證與成功輸出紀錄。

## Navigation

* [返回 Selected Works](../profile/works.md)
* [查看簡歷摘要](../profile/resume-lite.md)
* [返回根 README](../README.md)
