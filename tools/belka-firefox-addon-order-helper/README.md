...
本工具會在 `about:addons` 頁面中，透過 Firefox 內部的 `AddonManager` API 讀取目前安裝的使用者擴充套件。v0.1.6 起會明確從 `resource://gre/modules/AddonManager.sys.mjs` 匯入真正的 API，避免 Firefox 155 的 `about:addons` 頁面上同名全域物件造成衝突。工具會建立一個暫時的排序面板，讓你用拖曳或上移／下移按鈕調整順序，最後把結果寫入：
...
9. v0.1.6 起明確匯入 `AddonManager.sys.mjs`，避免 Firefox 155 的 `about:addons` 頁面中同名 `AddonManager` 全域物件造成誤判。
...
4. v0.1.6 已針對 Firefox 155 的 `about:addons` 執行環境調整 `AddonManager` 載入方式；仍建議先保留備份再套用。
5. 只要 `browser.uiCustomization.state` 與相關 Firefox 內部 API 維持相容，Firefox 109 之後的版本理論上仍可能可用，但不保證所有 ESR、Nightly 或衍生瀏覽器都一致。
...
1. 修正 Firefox 155 `about:addons` 頁面中的 `AddonManager` 全域名稱碰撞：不再直接使用頁面全域 `AddonManager`。
2. 改為透過 `ChromeUtils.importESModule("resource://gre/modules/AddonManager.sys.mjs")` 取得真正具有 `getAddonsByTypes()` 的 Firefox 內部 API。
3. `Services` 在可用時沿用現有全域；若未來不再直接提供，會嘗試從 `Services.sys.mjs` 匯入。
4. 加入內部 API 載入失敗的安全停止機制，載入失敗時不會修改偏好值。
